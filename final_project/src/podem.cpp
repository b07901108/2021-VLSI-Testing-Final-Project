...
#define CONFLICT 2
/*
int ATPG::tdf_LOS_podem(const fptr fault, int &current_backtracks) {
  ...
  } else if (fault->fault_type == STF) {// 1->0
...
int ATPG::podem(const fptr fault, int& current_backtracks)
{
    ...
    wptr faulty_wire;
    nptr faulty_node;
    if (faulty_node = fault->node) {
        if (fault->io) faulty_wire = faulty_node->owire.front();
        else faulty_wire = faulty_node->iwire[fault->index];
    } else {// PI/PO
        //??
    }
        ...
        if (total_attempt_num == 1) {
            ...
            for (i = 0; i < ncktin; i++) {
                ...
                case D_bar:
                    ...
                    break;
                case U:
                    //cktin[i]->value = rand() & 01;// ****** random fill ****** //here can try v2 gen
                    break; // random fill U
                }
            }
            //assign v1 done
            //then shift
            v2 = (cktin[0]->value == U)? (rand() & 01):cktin[0]->value;
...
// **************************************************************************
// Function   [ Atpg::SCOAPcontrollability ]
// Usage      [ find SCOAP controllability: cc0 cc1]
// Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
// **************************************************************************
void ATPG::SCOAPcontrollability()
{
    int i, j;
    int wireCount, nodeType, fanin;
    int cc0_min, cc1_min, cc0_sum, cc1_sum;
    nptr inputNode;

    wireCount = sort_wlist.size();

    for (i = 0; i < wireCount; ++i) {
        inputNode = sort_wlist[i]->inode.front();
        nodeType = inputNode->type;

        switch (nodeType) {
        case AND:
            fanin = inputNode->iwire.size();
            cc0_min = inputNode->iwire.front()->cc0;
            cc1_sum = 0;
            for (j = 0; j < fanin; ++j) {
                if (inputNode->iwire[j]->cc0 < cc0_min)
                    cc0_min = inputNode->iwire[j]->cc0;
                cc1_sum += inputNode->iwire[j]->cc1;
            }
            sort_wlist[i]->cc0 = cc0_min + 1;
            sort_wlist[i]->cc1 = cc1_sum + 1;
            break;

        case NOT:
            sort_wlist[i]->cc0 = inputNode->iwire.front()->cc1 + 1;
            sort_wlist[i]->cc1 = inputNode->iwire.front()->cc0 + 1;
            break;

        case OR:
            fanin = inputNode->iwire.size();
            cc0_sum = 0;
            cc1_min = inputNode->iwire.front()->cc1;
            for (j = 0; j < fanin; ++j) {
                if (inputNode->iwire[j]->cc1 < cc1_min)
                    cc1_min = inputNode->iwire[j]->cc1;
                cc0_sum += inputNode->iwire[j]->cc0;
            }
            sort_wlist[i]->cc0 = cc0_sum + 1;
            sort_wlist[i]->cc1 = cc1_min + 1;
            break;
        ...
        case NAND:
            ...
            sort_wlist[i]->cc0 = cc1_sum + 1;
            sort_wlist[i]->cc1 = cc0_min + 1;
            break;
            /*
        case OUTPUT:
            break;
...
// **************************************************************************
// Function   [ Atpg::SCOAPobservability ]
// Usage      [ find SCOAP observability: co ]
// Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
// **************************************************************************
void ATPG::SCOAPobservability()
{
    int i, j, k;
    int wireCount, nodeType, fanin, fanout;
    int co_temp;
    nptr outputNode;

    wireCount = sort_wlist.size();

    for (i = wireCount - 1; i >= 0; --i) {
        fanout = sort_wlist[i]->onode.size();

        for (j = 0; j < fanout; ++j) {
            outputNode = sort_wlist[i]->onode[j];
            nodeType = outputNode->type;

            switch (nodeType) {
            case AND:
                co_temp = outputNode->owire.front()->co + 1;
                fanin = outputNode->iwire.size();
                for (k = 0; k < fanin; ++k) {
                    if (outputNode->iwire[k] != sort_wlist[i]) {
                        co_temp += outputNode->iwire[k]->cc1;
                    }
                }
                if (co_temp < sort_wlist[i]->co) {
                    sort_wlist[i]->co = co_temp;
                }
                break;
            ...
            case OR:
                ...
                for (k = 0; k < fanin; ++k) {
                    if (outputNode->iwire[k] != sort_wlist[i]) {
                        co_temp += outputNode->iwire[k]->cc0;
                    }
                }
                if (co_temp < sort_wlist[i]->co) {
                    sort_wlist[i]->co = co_temp;
...
// **************************************************************************
// Function   [ Atpg::find_pi_assignment ]
// Usage      [ find an assignment value for PI ]
// Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
// **************************************************************************
/* backtrace to PI, assign a PI to achieve the objective.  Fig 9
 * returns the wire pointer to PI if succeed.
...
ATPG::wptr ATPG::find_pi_assignment(const wptr object_wire, const int& object_level)
{
    ...
    /* if not PI, backtrace to PI  Fig 9.3, 9.4, 9.5*/
        ...
        case NAND:
            if (object_level)
                new_object_wire = find_easiest_control(object_wire->inode.front(), object_level); // decision gate
            else
                new_object_wire = find_hardest_control(object_wire->inode.front(), object_level); // imply gate
            break;
        case NOR:
        case AND:
            // TODO  similar to OR and NAND but different polarity
            if (object_level)
                new_object_wire = find_hardest_control(object_wire->inode.front(), object_level);
            else
                new_object_wire = find_easiest_control(object_wire->inode.front(), object_level);
            break;
            //  TODO END
        case NOT:
        case BUF:
...
// **************************************************************************
// Function   [ Atpg::find_hardest_control ]
// Usage      [ find a hardest control wire meet specified objective level ]
// Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
// **************************************************************************
/* Fig 9.4 */
ATPG::wptr ATPG::find_hardest_control(const nptr n, const int& object_level)
{
    int i, nin;
    int hardCtlvalue = 0;
    wptr hardCtlwire = nullptr;

    if (object_level) {
        for (i = 0, nin = n->iwire.size(); i < nin; ++i) {
            if (n->iwire[i]->value == U) {
                if (n->iwire[i]->cc1 > hardCtlvalue) {
                    hardCtlvalue = n->iwire[i]->cc1;
                    hardCtlwire = n->iwire[i];
                }
            }
        }
    } else {
        for (i = 0, nin = n->iwire.size(); i < nin; ++i) {
            if (n->iwire[i]->value == U) {
                if (n->iwire[i]->cc0 > hardCtlvalue) {
                    hardCtlvalue = n->iwire[i]->cc0;
                    hardCtlwire = n->iwire[i];
                }
            }
        }
...
// **************************************************************************
// Function   [ Atpg::find_easiest_control ]
// Usage      [ find a easiest control wire meet specified objective level ]
// Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
// **************************************************************************
/* Fig 9.5 */
ATPG::wptr ATPG::find_easiest_control(const nptr n, const int& object_level)
{
    int i, nin;
    int easyCtlvalue = 2147483647; // max int
    wptr easyCtlwire = nullptr;

    if (object_level) {
        for (i = 0, nin = n->iwire.size(); i < nin; ++i) {
            if (n->iwire[i]->value == U) {
                if (n->iwire[i]->cc1 < easyCtlvalue) {
                    easyCtlvalue = n->iwire[i]->cc1;
                    easyCtlwire = n->iwire[i];
                }
            }
        }
    } else {
        for (i = 0, nin = n->iwire.size(); i < nin; ++i) {
            if (n->iwire[i]->value == U) {
                if (n->iwire[i]->cc0 < easyCtlvalue) {
                    easyCtlvalue = n->iwire[i]->cc0;
                    easyCtlwire = n->iwire[i];
                }
            }
        }
...
// **************************************************************************
// Function   [ Atpg::find_propagate_gate ]
// Usage      [ find a gate to propagate D value ]
// Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
// **************************************************************************
/* Find the eastiest propagation gate.   Fig 8.5, Fig 8.6 
 * returns the next gate with D or D_bar on inputs, U on output, nearest to PO
...
ATPG::nptr ATPG::find_propagate_gate(const int& level)
{
   ...
   * so that wires nearer to PO is checked earlier. */
    for (i = sort_wlist.size() - 1; i >= 0; i--) {
        ...
        /* gate outptu is U */
        /* a marked gate means it is on the path to PO */
        if ((sort_wlist[i]->value == U) && (sort_wlist[i]->inode.front()->is_marked())) { // Check if marked
            /*  cehck all gate intputs */
            for (j = 0, nin = sort_wlist[i]->inode.front()->iwire.size(); j < nin; j++) {
                w = sort_wlist[i]->inode.front()->iwire[j];
                /* if there is ont gate intput is D or D_bar */
...
// **************************************************************************
// Function   [ Atpg::trace_unknown_path ]
// Usage      [ find X-path ]
// Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
// **************************************************************************
/* DFS search for X-path , Fig 8.6
...
bool ATPG::trace_unknown_path(const wptr w)
{
    ...
    {
        nout = w->onode.size();
        nptr outputNode[nout];

        for (i = 0; i < nout; ++i) {
            outputNode[i] = w->onode[i];
        }

        sort(outputNode, outputNode + nout, ATPG::observability_compare);

        for (i = 0; i < nout; ++i) {
            if (outputNode[i]->owire.front()->value == U) // if unknown value exists
            {
                // down trace. if X-path exists, return TRUE.
                if (trace_unknown_path(outputNode[i]->owire.front()))
                    return true;
            }
        } // end of search
        return false; // if program runs out the for loop, there is no X-path
...
// **************************************************************************
// Function   [ Atpg::observability_compare ]
// Usage      [ compare observability in increase order ]
// Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
// **************************************************************************
bool ATPG::observability_compare(const nptr& node1, const nptr& node2) {
    return node1->owire.front()->co < node2->owire.front()->co;
}
