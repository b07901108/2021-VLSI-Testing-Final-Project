...
public:
  ...
  //            [ find SCOAP controllability: co ]
  // Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
  void SCOAPcontrollability();
  void SCOAPobservability();
  // **************************************************************************
...
private:
  ...
  // **************************************************************************
  // Functions  [ Atpg::find_pi_assignment ]
  //            [ Atpg::find_hardest_control ]
  //            [ Atpg::find_easiest_control ]
  //            [ Atpg::find_propagate_gate ]
  //            [ Atpg::trace_unknown_path ]
  //            [ Atpg::observability_compare ]
  // Usage      [ find an assignment value for PI ]
  //            [ find a hardest control wire meet specified objective level ]
  //            [ find a easiest control wire meet specified objective level ]
  //            [ find a gate to propagate D value]
  //            [ find X-path]
  //            [ compare observability in increase order]
  // Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
  wptr find_pi_assignment(const wptr, const int &);
  wptr find_hardest_control(const nptr, const int &);
  wptr find_easiest_control(const nptr, const int &);
  nptr find_propagate_gate(const int &);
  bool trace_unknown_path(const wptr);
  static bool observability_compare(const nptr &, const nptr &);
  // **************************************************************************
  ...
  public:
    ...
    // **************************************************************************
    // Class      [ Atpg::WIRE ]
    // Usage      [ new WIRE Class support SCOAP]
    // Date       [ Chi-Jhe Li Ver.1.0 2021/05/29 ]
